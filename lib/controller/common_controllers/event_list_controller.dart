...
class EventListController extends GetxController {
  ...
  Future<void> fetchEvents() async {
    try {
      ...
      if (organisationId == null) {
        print('⚠️ No organisation selected, skipping event loading');
        events.clear();
      ...
      print(
          '✅ Loaded ${events.length} events for organisation: $organisationId');
    } catch (e) {
  ...
  void onInit() {
    ...
    ever(authController.organisationId, (orgId) {
      if (orgId != null) {
        print('🔄 Organisation changed, reloading events for: $orgId');
        fetchEvents().then((_) {
          filteredEvents.assignAll(events);
          sortEvents(SortType.dateNewest);
        }).catchError((error) {
        ...
        print('🧹 Clearing events - no organisation selected');
        events.clear();
        filteredEvents.clear();
      }
    });
